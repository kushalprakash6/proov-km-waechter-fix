...
def car_wear(car: dict) -> float:
    """Return the wear percentage for a car. Returns 0.0 if no service reading exists."""
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Return a summary dict with car count, number due, and average wear percentage."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
